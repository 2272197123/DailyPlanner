...
def _build_prompt(for_date: str, day_mode: str, feedback: str, goal_id: str | None, db) -> str:
    ...
    try:
        mode_cfg = json.loads(mode_cfg_raw) if mode_cfg_raw else {
            ...
            "recovery": {"label": "🌱 恢复", "factor": 0.25, "hours": "1-2h", "desc": "慢下来。"},
        }
    except Exception:
        mode_cfg = {"full": {"label": "🔋 完整", "factor": 1.0}, "minimum": {"label": "⚡ 最低", "factor": 0.55}, "recovery": {"label": "🌱 恢复", "factor": 0.25}}

    current_mode = mode_cfg.get(day_mode, mode_cfg.get("full", {}))
    mode_lines = []
    for k in ("full", "minimum", "recovery"):
        mc = mode_cfg.get(k, {})
...
def generate_daily_plan(for_date: str, day_mode: str = "full", feedback: str = "", goal_id: str | None = None, db=None) -> dict:
    """
    调用 AI 生成每日计划。
    ...
    try:
        ...
        # 记录失败回复（截断）
        _log_ai_request(for_date, prompt, content[:500], usage, elapsed, success=False, error=str(e), db=db)
    ...
    # 记录成功请求
    _log_ai_request(for_date, prompt, content[:500], usage, elapsed, success=True, db=db)
...
def _log_ai_request(for_date: str, prompt: str, response_snippet: str, usage: dict, elapsed: float, success: bool, error: str = "", db=None):
    ...
    try:
        ...
        db.commit()
        db.execute(
            "INSERT INTO ai_requests (target_date, success, prompt_tokens, completion_tokens, total_tokens, elapsed_sec, error_msg, prompt_snippet, response_snippet) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (for_date, 1 if success else 0,
